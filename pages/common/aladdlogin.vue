	...
	<view class="login-wrap">
		...
		<view class="login-container text-white">
			<view class="d-flex-between-center">
				<text class="d-block font-size-48 my-8 font-weight-bold linear-gradient-text" style="line-height: 66rpx;">{{i18n.walletAddress}}{{i18n.login}}</text>
			...
			<view>
				...
				<input type="text" v-model="user_string" class="login-input"
						:placeholder="i18n.walletAddress" confirm-type="next" @confirm="passwordFocus=true">
				</view>
				...
				<view class="login-btn-group">
					<view class="d-flex mt-60 mb-16">
						...
						<!-- {{i18n.noaccount + i18n.go}} -->
						<navigator url="/pages/common/login" class="ml-4 text-warning" style="text-aling:right">{{fund.account}}{{i18n.login}}</navigator>
		...
		methods: {
			...
			login() {
				this.$u.throttle(async () => {
					...
					if (!user_string) {
						this.$utils.showToast(this.i18n.pleaseEnterWalletAddress) //为填写地址提示
						return false
					}					
					try {
