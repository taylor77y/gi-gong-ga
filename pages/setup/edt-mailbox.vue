	...
	<view class="container">
		...
		<view class="con-box">
			...
			<view class="item">
				...
				<view class="f-input">
					<input style="width: 90%;" v-model="mailbox" :placeholder="i18n.qsryxdz" placeholder-style="color: #B0B3BA;font-size: 32rpx" />
				</view>
			</view>
			<view class="item">
				<view class="title1">
				...
				<view class="f-input">
					<input style="width: 25%;" type="number" v-model="code" :placeholder="i18n.yxyzm" placeholder-style="color: #B0B3BA;font-size: 32rpx" />
	...
	export default {
		...
		methods: {
			...
			sendAdd() {
				this.min = 60
				const { mailbox } = this
				if (this.$utils.testEmail(mailbox)) {
			...
			getAdd() {
				...
				this.$u.api.user.checkSmsCode(mailbox, code).then(res=>{
					if(res.status == "SUCCEED"){
