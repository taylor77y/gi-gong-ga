<template>
	<view class="container">
		...
		<head-title :title="i18n.xgjymm" />
		<view class="tips">
			<!-- *为了您的资产安全，登录密码修改后24h以内不允许提现。 -->
		</view>
		<view class="bank-list">
			<view class="item" v-for="(item, index) in bankInfo" :key="index">
				...
				<view class="f-input">
					<input type="number" v-model="item.value" :placeholder="item.pla"
...
<script>
	...
	export default {
		...
		onLoad() {
            this.bankInfo = this.i18n.bankInfo1
		},
		computed: {
            i18n() {
		...
		methods: {
            getAdd() {
				...
				if (this.bankInfo[0].value.length < 6 || this.bankInfo[1].value.length < 6) {
					this.$utils.showToast(this.i18n.qsrlwszjymm)
					return
				}
				...
				this.$u.api.user.resetPayPass(member,oldPass,newPass).then(res=>{
