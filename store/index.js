...
const store = new Vuex.Store({
	state: {
		...
		// }],
		baseUrl: process.env.NODE_ENV === 'development' ? '/api' : "https://www.swapcion.com/api",
		baseDomain:"ws://qcapi.obk3.com/wss/", // wss 链接
		...
		countryCode: uni.getStorageSync('chooseCode') || 86, //国家码
	...
	actions: {
		login({
			...
			return new Promise((resole, reject) => {
				...
				console.log(params)
				const { user_string:phMail, password } = params.data
				const temp = {
					phMail,
					password: md5Libs.md5(password),
					areaCode: 86
				}
				_this.$u.api.user.login(temp).then(res => {
					console.log("登录结果",res)
					if(res.status == "SUCCEED"){
						uni.setStorageSync('userId',res.result.id)
						//保存token至缓存
						commit('saveUser', res.result.token)
						resole(true)
					}
				}).catch((res) => {
					reject(res)
	...
	mutations: {
		...
		setCountryCode(state, countryCode) {
			...
			uni.setStorageSync('chooseCode',state.countryCode)
		},
		// 选择法币货币
		setFackCur(state, fackCur) {
			state.fackCur = fackCur
			uni.setStorageSync('fackCur',state.fackCur)
		},
		// 买入 卖出状态
		setSellCode(state, sellCode) {
			state.sellCode = sellCode
		},
		// 底部栏状态
		setTabarCode(state, tabarCode) {
			state.tabarCode = tabarCode
			uni.setStorageSync('tabarCode',state.tabarCode)
		},
		// 汇率
		setRate(state, rate) {
			state.rate = rate
